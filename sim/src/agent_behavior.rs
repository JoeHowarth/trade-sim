use crate::{transition_state, AgentState};
...
pub fn decide_single_good_multi_step(
    ...
    info!(
        "Agent {} performing {}. Expecting profit of {} after {} actions",
        agent,
        action.name(),
        last_state.money - *money,
        DEPTH
    );
    match action {
...
fn dfs(
    ...
    let (_, _, linked_cities): (_, _, &LinkedCities) =
        context.get(state.location.city_res()?.entity)?;
    actions.extend(linked_cities.0.iter().map(|ch| {
    ...
    let next_states = actions.iter().filter_map(|action| {
    ...
    next_states
        .filter_map(|(next_action, next_state)| {
...
fn decide_single_simple(
    (e_agent, agent, cargo, _money, pos): (
        Entity,
        &Agent,
        &Cargo,
        &Money,
    ...
    let (_, src_city, src_market, links): (
        _,
        &City,
        &MarketInfo,
        &LinkedCities,
    ) = cities_q.get(pos.city_res()?.entity)?;
    let linked_markets: Vec<(CityHandle, &MarketInfo)> = links
        .0
        .iter()
        .map(|ch| {
            (
                *ch,
                cities_q
                    .get_component::<MarketInfo>(ch.entity)
                    .unwrap(),
            )
        })
        .collect();
    ...
    linked_markets
        ...
        .and_then(|(city_with_highest_price, highest_market)| {
            if highest_market.current_price() > src_market.current_price() {
                ...
                info!(
                    "Agent {} buying {} at {} in {} and moving to {}, expecting profit of {}",
                    agent,
                    good,
                    src_market.current_price(),
                    src_city,
                    city_with_highest_price,
                    highest_market.current_price() - src_market.current_price()
                );
                return Some(());
            }
            linked_markets
                ...
                .map(|(lowest_city, lowest_market)| {
                    ...
                    info!(
                        "Agent {} moving from {} to {} in order to buy {} at lowest price of {}",
                        agent,
                        src_city,
                        lowest_city,
                        good,
                        lowest_market.current_price()
                    );
                    movement.send(Movement {
                        from: pos.clone(),
        ...
        .unwrap_or_else(|| {
            info!(
                "Agent {} cannot move because no linked cities from {}",
                agent, src_city
            )
        });
