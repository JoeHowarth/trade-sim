...
pub fn transition_movement(
    ...
    if movement.from == pos {
        ...
        Err(anyhow!("expected Movement action 'from' to be equal to position prior to movement: {:?}, {}", movement, pos))
    }
}
...
pub fn movement(
    ...
    for m in move_reader.iter() {
         let _ = pos.get_mut(m.entity)
            .context(anyhow!("expected Movement action to be valid, {:?}", &m))
            .map(|mut pos| {
