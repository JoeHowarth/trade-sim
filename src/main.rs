...
use crate::init::Cli;
use sim::{agent_behavior, order_clearing::Failed};
use structopt::StructOpt;
use types::{
    agent::{Agent, Cargo, GraphPosition},
...
fn main() -> Result<()> {
    ...
    let input =
        init::get_input(args).context("Failed to get input")?;
    let (state_tx, state_rx) = mpsc::unbounded_channel();
    let (save_signal_sender, save_signal_recv) =
...
fn build_app(
    ...
    .insert_resource(ScheduleRunnerSettings {
        run_mode: RunMode::Loop {
            wait: Some(Duration::from_millis(
                input.settings.loop_rate.clone(),
            )),
        },
    })
    ...
    .add_stage(
        ...
        SystemStage::single_threaded()
            ...
            .with_system(wrap(
                sim::order_clearing::clear_orders.system(),
            )),
    )
    .add_stage(
        "final-work",
